...
def test_get_trail_conditions_passes_canonical_name_and_returns_quality():
    condition = SimpleNamespace(ski_quality='good')
    with patch.object(service._trail_integration, 'get_trail_conditions',
                      return_value=condition) as mock_lookup:
        result = service.get_trail_conditions('Hyland Lake Park Reserve')
    assert result == 'good'
    mock_lookup.assert_called_once_with('Hyland Lake Park Reserve')


def test_get_trail_conditions_returns_none_when_no_report():
    with patch.object(service._trail_integration, 'get_trail_conditions',
                      return_value=None):
        assert service.get_trail_conditions('Theodore Wirth Park') is None


def test_get_trail_conditions_returns_none_on_integration_error():
    with patch.object(service._trail_integration, 'get_trail_conditions',
