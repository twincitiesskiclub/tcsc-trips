...
@patch("app.verify.providers.requests.post")
def test_verify_check_returns_false_on_404(mock_post):
...
def test_twilio_verify_start_raises_provider_error_when_credentials_missing():
    from app import create_app
    app = create_app()
    app.config.update(
        TESTING=True,
        TWILIO_ACCOUNT_SID="AC_test", TWILIO_API_KEY_SID=None,
        TWILIO_API_KEY_SECRET="secret", TWILIO_VERIFY_SERVICE_SID="VA_test",
        TWILIO_MESSAGING_SERVICE_SID="MG_test", RESEND_API_KEY="re_test",
    )
    with app.app_context():
        with pytest.raises(ProviderError) as exc:
            providers.twilio_verify_start("+16128677165")
        assert "credentials are not configured" in str(exc.value).lower()


def test_twilio_send_sms_raises_provider_error_when_credentials_missing():
    from app import create_app
    app = create_app()
    app.config.update(
        TESTING=True,
        TWILIO_ACCOUNT_SID="AC_test", TWILIO_API_KEY_SID="SK_test",
        TWILIO_API_KEY_SECRET=None, TWILIO_VERIFY_SERVICE_SID="VA_test",
        TWILIO_MESSAGING_SERVICE_SID="MG_test", RESEND_API_KEY="re_test",
    )
    with app.app_context():
        with pytest.raises(ProviderError) as exc:
