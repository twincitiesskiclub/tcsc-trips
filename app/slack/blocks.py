...
def build_weekly_summary_blocks(practices: list[PracticeInfo]) -> list[dict]:
    ...
    blocks.append({
        ...
        "text": {
            "type": "plain_text",
            "text": ":ski: This Week's Practices"
        }
    })
    ...
    for practice in practices:
        ...
        practice_text = f"*{date_str}*\n"
        practice_text += f"Location: {location}\n"

        if practice.activities:
            activity_names = ", ".join([a.name for a in practice.activities])
            practice_text += f"Activities: {activity_names}\n"
        ...
        if practice.status == PracticeStatus.CANCELLED:
            blocks.append({
                ...
                "elements": [{
                    "type": "mrkdwn",
                    "text": f":x: *CANCELLED* - {practice.cancellation_reason or 'No reason provided'}"
                }]
            })
