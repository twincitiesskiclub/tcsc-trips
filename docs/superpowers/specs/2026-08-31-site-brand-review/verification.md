...
|---|---|---|---|
| L-001 | fixed | verified | rework 8684377 replaced `text-ink/75` with `text-slate` at sponsors.astro:102; tree grep shows zero ink alphas in src; built sponsors html has none; verify2/sponsors-default-desktop.png renders the priorities intro in slate |
...
| L-015 | fixed | verified | rework 8684377 put sponsors back on roles: :52 and :87 `type-lede`, :82 `type-caption`, disclosure dropped its `text-base`. no `leading-relaxed` or size utility left in sponsors.astro; builds compile the roles; other sites held |
...
| L-075 | fixed | verified | rework 8684377 added `.prose-statement > article > p:first-child` to both statement rules in global.css; compiled css in builds/open carries the selector with the clamp and navy color; verify2/about-closed-desktop.png shows the founding paragraph at statement scale, navy, 44ch |
...
| L-083 | fixed | verified | rework 8684377 added `'logo-mobile': '13.5rem'` to the tailwind.config.ts maxWidth map and SponsorWall.astro:52 uses `max-w-logo-mobile`; compiled css carries the token; no `max-w-[` left in src outside the sacred files |
...
| L-007 | fixed | verified | rework 8684377 deleted loppet-skijor.yaml and loppet-skijor.jpg (restorable from git) and CONSENT.md:37 records the pull with the restore path; content.config.ts untouched; zero loppet-skijor references in src or any build; verify2/community-default-desktop.png wall is clean |
...
| L-028 | fixed | verified | rework 8684377 put one alt on both call sites: about.astro:76 and sponsors.astro:44 both read "About 20 members in matching club caps posed around a bench after a golden-hour rollerski, skis and helmets at their feet" ("About 20" per the L-093 numeral style); both built pages carry it |
...
re-verified against 8684377, the rebuilt builds, fresh verify2 screenshots (sponsors, about-closed, community, home-open at both viewports), and the tests (registrationCopy 22 pass, sponsors-page pass).

1. [sections; L-001, L-015] resolved. sponsors.astro:102 is `text-slate`, :52/:87 are `type-lede`, :82 is `type-caption`; no `text-ink/` or `leading-relaxed` left in the file.
2. [sections; L-083] resolved. `logo-mobile: 13.5rem` token in tailwind.config.ts, `max-w-logo-mobile` at SponsorWall.astro:52; no `max-w-[` in src outside the sacred files.
3. [foundations L-075] resolved. both statement rules also match `.prose-statement > article > p:first-child`; the about founding paragraph renders at the statement clamp, navy, 44ch (verify2/about-closed-desktop.png).
4. [copy L-007] resolved. loppet-skijor.yaml and .jpg removed, restore note recorded at CONSENT.md:37, content.config.ts untouched, no reference remains in src or builds.
5. [copy L-028] resolved. one alt on both pages, "About 20" numeral style in both.
6. [copy; registration table] resolved. stripSubhead open branch returns `Registration is open. ${ABILITY}`; tests/registrationCopy.test.mjs updated in the same commit and passing; the strip renders the full sentence in verify2/home-open-desktop.png.
...
re-verify round: rework landed in 8684377. i re-checked the eight rework (B) items against that diff, the merged tree, fresh captures (`PORT=4406 node screenshot.mjs verify2b`, screens/verify2b/), a live playwright measurement of the injected venue anchor at 390, and full runs of test:refinement (76 pass, brandPrimitives now included) and test:sponsors (24 pass). updated verdicts are marked "rework:" in the evidence.
...
| L-049 | fixed (global.css:114, da8fd62) | verified | rework 8684377: the venue-link rule moved to global.css unscoped and compiles as `[data-location] a[data-source-link]{...min-height:2.75rem...}` (builds/open/_astro css confirms). measured live with playwright at 390 on the served open build: the injected Theo anchor (no astro-cid) computes inline-flex, min-height 44px, and a 44.0px bounding box on both / and /about. GV-1 is cured with it. the three parts that already held (cursor, closers, fever hover) re-checked and hold. the anchor's width is 41.8px; the shipped rule is exactly what GV-1 and the contract prescribe (min-height only), recording the width for completeness. |
...
| L-112 | fixed (brandPrimitives.test.mjs, 8fe895a) | verified | the dist-grep test exists and passes (5/5 locally): text-[Npx]/tracking-[ ban, one seam per band, coral rule singular, footer order, oklch containment. rework 8684377: wired into test:refinement in site/package.json; the suite now runs 76 tests (71 + these 5), all pass. |
...
| L-120 | fixed (Lightbox.astro:10, a9b609b) | verified | `bg-navy-deep/95` (in the focus map via `[data-lightbox]`, no inline override), inline chevron strokes (`M15 18l-6-6 6-6` / `M9 18l6-6-6-6`) in min-h-11 min-w-11 hit areas, no borders, paper/75→mint, caption mt-3 under the image. home-lightbox-desktop.png confirms. rework 8684377: the caption wrapper's `max-w-3xl` is now `max-w-prose` (verify2b/home-lightbox-desktop.png, caption still centers). |
| L-008 | fixed (HeroHome.astro:62, eede554, a34315c) | verified | content block moves to `lg:col-start-5 lg:col-span-8`; md crop `object-[100%_15%]`; mobile kept. fresh home-hero-{mobile,tablet,desktop}.png: text sits over trail and torsos, all faces clear at 390, 768, 1440. rework 8684377: the `.hero-title` font-size/nowrap override is deleted; the h1 renders `.type-display` (6rem) and wraps to two balanced lines at 1440 (verify2b/home-hero-desktop.png: "Twin Cities / Ski Club", all four faces still clear from col-start-5). GV-4 is moot: site and contract now agree. |
| L-009 | fixed (CoachEntry.astro:73, 1a90aab, 195fc6a) | regressed | the layout holds (max-w-site 12-col grid, attention-cropped square, greg leads; verify2b/coaches-default-desktop.png clean). but rework 8684377 removed the `.coach-name` override (curing violation 2) and the accepted fallback does not work: at 390, `.type-h2` renders 32px beside the 240px portrait, "Rebecca" (scrollWidth 152 vs clientWidth 86) and "Michael" (133 vs 86) cannot wrap as single words, the names clip at the viewport edge and the page gains a horizontal scroll (document.scrollWidth 432 in a 390 viewport). measured live; visible in verify2b/coaches-default-mobile.png. still open, see rework item 4. |
...
| L-054 | fixed (global.css:145, 4989a94d) | verified | rework 8684377: trips/index.astro drops ProseColumn and mounts TripsTable in `<section class="gutter mx-auto max-w-site band-y-sm">`. verify2b/trips-populated-desktop.png: the three-column ledger spans the 1280 container, dates at the left gutter, where at the right edge. the reading column, one left edge, and the wrapper deletions still hold. |
| L-055 | fixed (sponsors.astro:34, 593cd3a) | verified | rework 8684377: priorities intro is `type-lede text-slate` (line 102), impact intro `type-lede text-paper`, recognition body `type-lede text-ink`, caption `type-caption text-slate`, disclosure dropped `text-base`. grep: zero `text-ink/` and zero `leading-relaxed` in sponsors.astro; both builds render `type-lede text-slate`. test:sponsors 24 pass. verify2b/sponsors-default-{desktop,mobile}.png: the structure that was already right still holds and the intro reads slate. |
| L-056 | fixed (SponsorWall.astro:52, 9692ab3) | verified | ruled row per tier (border-y hairline), tier label in the seam voice at left, logos left-aligned at every width, max-h-16 under sm, logo-lg/md/sm tokens in tailwind.config.ts. sponsors-default-{desktop,mobile}.png: two logos on one dignified row, nothing centered. rework 8684377: the arbitrary width is now `max-w-logo-mobile` with the token in tailwind.config.ts maxWidth; grep shows zero `max-w-[` left in src. |
...
counts after rework 8684377: 64 verified, 0 not fixed, 1 regressed (L-009 reopened: the override removal exposed a mobile name overflow; everything else from rework (B) resolved and all four violations cured).

## violations (B)

1. CURED in 8684377. eede554 (sections, L-008) had set `.hero-title { font-size: clamp(4rem, 6.25vw, 5.25rem); white-space: nowrap; }` in HeroHome.astro's style block; the rework deleted the override, `.type-display` governs, and grep finds no font-size in the component.
2. CURED in 8684377 (at the source). 195fc6a (sections, L-009) had set `.coach-name { font-size: ...; line-height: 1.05 }` in CoachEntry.astro's style block; the rework deleted it and no font-size remains in any component style block. the visible fallout (mobile name overflow) is tracked as reopened L-009, not as a rule violation.
3. CURED in 8684377. 593cd3a (sections, L-055) had put `text-ink/75` on sponsors.astro:102; the line is now `type-lede text-slate` and no ink alpha remains anywhere in src.
4. CURED in 8684377. 9692ab3 (sections, L-056) had added `max-w-[13.5rem]` in SponsorWall.astro:52; it is now `max-w-logo-mobile` with the token in tailwind.config.ts, and no `max-w-[` remains in src.
...
re-verified against 8684377, fresh verify2b captures, and live measurement. one line per item:

1. [sections, L-055] resolved. sponsors.astro:102 is `type-lede text-slate`; all four call-site chains replaced with type roles; zero `text-ink/` or `leading-relaxed` in the file; test:sponsors 24 pass.
2. [sections, L-054] resolved. trips table mounts in `gutter mx-auto max-w-site band-y-sm`; the ledger spans the container in verify2b/trips-populated-desktop.png.
3. [sections, L-008] resolved. `.hero-title` override deleted; the h1 wraps to two balanced type-display lines at 1440 and all faces stay clear (verify2b/home-hero-desktop.png).
4. [sections, L-009] resolved in round 2 (by the review runner, commit follows): the coach side-by-side grid breakpoint moved from min-[360px] to min-[30rem], so phones stack photo then full-width name. measured live: document.scrollWidth 390 at 390 and 480 at 480 on /coaches; after/coaches-default-mobile.png recaptured. no font-size in component style blocks. original finding text: still-open. the override was removed, but the "accept type-h2's mobile clamp" option fails in practice: single-word names cannot wrap, so at 390 "Rebecca" and "Michael" clip at the viewport edge and the page scrolls horizontally (document.scrollWidth 432 vs 390; measured live, visible in verify2b/coaches-default-mobile.png). what remains: take the role-level option instead. either add a vw term to `.type-h2`'s mobile clamp in global.css (one definition for the role, e.g. `clamp(1.375rem, 5.5vw + 0.1rem, 2rem)` under the md breakpoint) or rework the sub-360 name column so the h2 gets more than ~134px. no font-size in component style blocks, and re-measure document.scrollWidth == 390 on /coaches at 390 after.
5. [sections, L-056] resolved. `max-w-logo-mobile` with the token in tailwind.config.ts; no `max-w-[` left in src. (also closes rework (A) item 2.)
6. [motion-components, L-049] resolved. rule in global.css unscoped; live playwright at 390 measures the injected Theo anchor at 44px tall on / and /about.
7. [motion-components, L-112] resolved. brandPrimitives wired into test:refinement; suite runs 76 tests, all pass.
8. [motion-components, L-120] resolved. lightbox caption is max-w-prose.

process note: 8684377 edits site/package.json (the test:refinement list). that edit is the one item 7 instructed, adds no dependency, and is recorded here as sanctioned.

## deferred (round limit)

fast-follow items the gestalt reviewer explicitly marked as not blocking merge; none argue against the branch:
- GV-2 [copy, P3]: sponsors closing band repeats "what comes next" as both seam and h2; give the seam a chapter name or let it be the h2.
- GV-3 [sections, P3]: sponsors ledger detail columns sit on three axes (368/643/663); one axis per page.
- GV-5 [sections, P3]: community groups uncapped; apply the hub rule (extra training fun shows one row plus link, others near four rows).
- GV-6 [motion-components, P3]: the fever cell hover changes only the ♪ alpha; make the hover visible or record the ♪ as the affordance.
- flask-side follow-up (not a site row): the groomed line renders at 11px from the api payload; recorded by the gestalt reviewer.
