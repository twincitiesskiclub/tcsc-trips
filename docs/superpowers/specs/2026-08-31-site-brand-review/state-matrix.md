...
- Screenshots are not committed (`screens/` is gitignored in this directory; a phase is about 100MB and would double the repo Render clones on every deploy). They live on the review machine at `/workspace/tcsc-trips/docs/superpowers/specs/2026-08-31-site-brand-review/screens/<phase>/`; every agent reads them from there. Rob reviews the result on the Render preview.
